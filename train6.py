...
def train_model(model, dataloader, criterion, optimizer, num_epochs=30, model_fold=0):
    ...
    with open(log_file, 'a') as f:
                f.write(f'\n------------------------------------------------------------------------------------\n')
    test_dataset = CustomImageDataset(annotations_file= f'training/fold{model_fold}_test.txt', img_dir='training/images', transform=transform)
...
for i in model_nths :
    ...
    train_dataset = CustomImageDataset(annotations_file=f'training/fold{i}_train.txt', img_dir='training/images', transform=transform)
    dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=1)
    print(f"Training model {i}") 
    train_model(model, dataloader, criterion, optimizer, num_epochs=1, model_fold=6)
